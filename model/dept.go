...
func (e *Dept) Create() (Dept, error) {
	...
	if int(e.ParentId) != 0 {
		var deptP Dept
		base.DB.TestDB.Table(e.TableName()).Where("dept_id = ?", e.ParentId).First(&deptP)
	...
	if err := base.DB.TestDB.Table(e.TableName()).Where("dept_id = ?", e.DeptId).Update(mp).Error; err != nil {
		err := result.Error
		return doc, err
	}
	doc = *e
...
func (e *Dept) Update(id int) (update Dept, err error) {
	if err = base.DB.TestDB.Table(e.TableName()).Where("dept_id = ?", id).First(&update).Error; err != nil {
		return
	}
...
func (e *Dept) Delete(id int) (success bool, err error) {
	if err = base.DB.TestDB.Table(e.TableName()).Where("dept_id = ?", e.DeptId).Delete(&Dept{}).Error; err != nil {
		success = false
		return
	}
	success = true
