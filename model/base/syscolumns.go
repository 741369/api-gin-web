...
func (e *SysColumns) GetList() ([]SysColumns, error) {
	...
	table := DB.TestDB.Select("*").Table("sys_columns")
...
func (e *SysColumns) Create() (SysColumns, error) {
	var doc SysColumns
	result := DB.TestDB.Table("sys_columns").Create(&e)
	if result.Error != nil {
		err := result.Error
		return doc, err
	}
...
func (e *SysColumns) Update() (update SysColumns, err error) {
	if err = DB.TestDB.Table("sys_columns").First(&update, e.ColumnId).Error; err != nil {
		return
	}

	//参数1:是要修改的数据
	//参数2:是修改的数据
	if err = DB.TestDB.Table("sys_columns").Model(&update).Updates(&e).Error; err != nil {
		return
	}
