...
func (e *DictData) Update(id int) (update DictData, err error) {
	if err = base.DB.TestDB.Table(e.TableName()).Where("dict_code = ?", id).First(&update).Error; err != nil {
		return
	}
...
func (e *DictData) Delete(id int) (success bool, err error) {
	if err = base.DB.TestDB.Table(e.TableName()).Where("dict_code = ?", id).Delete(&DictData{}).Error; err != nil {
		success = false
		return
	}
	success = true
