...
func (rm *RoleMenu) Get() ([]RoleMenu, error) {
	var r []RoleMenu
	table := base.DB.TestDB.Table("sys_role_menu")
	if rm.RoleId != 0 {
...
func (rm *RoleMenu) GetPermis() ([]string, error) {
	var r []Menu
	table := base.DB.TestDB.Select("sys_menu.permission").Table("sys_role_menu").Joins("left join sys_menu on sys_menu.menu_id = sys_role_menu.menu_id")
...
func (rm *RoleMenu) GetIDS() ([]MenuPath, error) {
	var r []MenuPath
	table := base.DB.TestDB.Select("sys_menu.path").Table("sys_role_menu")
...
func (rm *RoleMenu) DeleteRoleMenu(roleId int) (bool, error) {
	if err := base.DB.TestDB.Table("sys_role_dept").Where("role_id = ?", roleId).Delete(&rm).Error; err != nil {
		return false, err
	}
	if err := base.DB.TestDB.Table("sys_role_menu").Where("role_id = ?", roleId).Delete(&rm).Error; err != nil {
		return false, err
	}
	var role SysRole
	if err := base.DB.TestDB.Table("sys_role").Where("role_id = ?", roleId).First(&role).Error; err != nil {
		return false, err
	}
	sql3 := "delete from casbin_rule where v0= '" + role.RoleKey + "';"
	base.DB.TestDB.Exec(sql3)
...
func (rm *RoleMenu) BatchDeleteRoleMenu(roleIds []int) (bool, error) {
	if err := base.DB.TestDB.Table("sys_role_menu").Where("role_id in (?)", roleIds).Delete(&rm).Error; err != nil {
		return false, err
	}
	var role []SysRole
	if err := base.DB.TestDB.Table("sys_role").Where("role_id in (?)", roleIds).Find(&role).Error; err != nil {
		return false, err
	}
	sql := ""
...
func (rm *RoleMenu) Insert(roleId int, menuId []int) (bool, error) {
	...
	var menu []Menu
	if err := base.DB.TestDB.Table("sys_menu").Where("menu_id in (?)", menuId).Find(&menu).Error; err != nil {
		return false, err
	}
...
func (rm *RoleMenu) Delete(RoleId string, MenuID string) (bool, error) {
	...
	table := base.DB.TestDB.Table("sys_role_menu").Where("role_id = ?", RoleId)
	if MenuID != "" {
