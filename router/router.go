...
func Load(g *gin.Engine, mw ...gin.HandlerFunc) *gin.Engine {
	...
	apiv1 := g.Group("/api/v1")
	{
		...
		apiv1.GET("/db/tables/page", system.GetDBTableList) // 获取数据库表结构
		apiv1.GET("/db/columns/page", system.GetDBColumnList)

		apiv1.GET("/sys/tables/info/:tableId", system.GetSysTables)
		apiv1.GET("/sys/tables", system.GetSysTableList)
		apiv1.POST("/sys/tables", system.InsertSysTable)
		apiv1.PUT("/sys/tables", system.UpdateSysTable)
		apiv1.DELETE("/sys/tables/:tableId", system.DeleteSysTables)
