		...
		if os.Getenv("ENV_GO") == "" || os.Getenv("ENV_GO") == "dev" || os.Getenv("ENV_GO") == "pre" {
			if pad.Right(method, 5, "") != "OPTIONS" {
				...
				returnData := map[string]interface{}{
					...
					"app-type":          c.Request.Header.Get("app-type"),
					"app-version":       c.Request.Header.Get("app-version"),
					"sdk-version":       c.Request.Header.Get("sdk-version"),
					"user-id":           c.Request.Header.Get("user-id"),
					"session-id":        c.Request.Header.Get("session-id"),
					"guid":              c.Request.Header.Get("guid"),
					"partner-id":        c.Request.Header.Get("partner-id"),
					"app-id":            c.Request.Header.Get("app-id"),
					"app-name":          c.Request.Header.Get("app-name"),
					"mobile-version":    c.Request.Header.Get("mobile-version"),
